...
export default function WalletPage() {
  ...
  const handleWithdraw = async () => {
    ...
    if (amt > balance) { setWithdrawError("Insufficient balance"); return; }
          ...
          <div className="rounded-2xl border border-border bg-card p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={openWithdraw} disabled={balance <= 0 || hasPending}
                      ...
                      <tr key={tx.id} className="border-b border-border last:border-0 hover:bg-accent/50 transition-colors">
                        <td className="px-6 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">{tx.type === "DEPOSIT" ? "Deposit" : "Withdrawal"}</span>
                          </div>
                        </td>
                        <td className="px-6 py-3">
                          <span className={`font-medium ${tx.type === "DEPOSIT" ? "text-emerald-500" : "text-foreground"}`}>
      ...
      {withdrawOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-2xl border border-border bg-card p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-display text-lg font-semibold">Withdraw Funds</h3>
            ...
            {withdrawSuccess ? (
              ...
              <>
                <p className="text-sm text-muted-foreground">Available balance: <span className="font-medium text-foreground">{sym}{balance.toFixed(2)}</span></p>
                {withdrawError && (
                  <div className="flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="relative mt-1">
                      <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-muted-foreground/60 font-medium">{sym}</span>
                      <input type="number" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} placeholder="0.00" min="0" max={balance} step="0.01"
                        className="w-full rounded-lg border border-border pl-8 pr-4 py-2.5 text-sm placeholder:text-muted-foreground/40 focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand/50 transition-all bg-background" />
                    </div>
                  </div>
                  <div>
